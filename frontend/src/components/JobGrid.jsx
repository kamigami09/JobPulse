...
export default function JobGrid({ refreshKey, onAddJob, statusFilter }) {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className={`transition-colors ${row.original.status === 'Interviewing' ? 'text-indigo-400 hover:text-indigo-300' : 'text-zinc-600 hover:text-indigo-400'}`}
  ...
  return (
    <>
    {prepJob && (
      <PrepChecklist job={prepJob} onClose={() => setPrepJob(null)} />
    )}
